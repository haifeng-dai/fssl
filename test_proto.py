...
class Global:
    ...
    def evaluate(
        ...
        mean_prototype_mask=None,
    ):
        """在测试集上评估分类头和按样本数加权聚合的均值原型。"""
        self.model.load_state_dict(params)
        self.model.eval()
        model_correct = 0
        prototype_correct = 0
        prototypes = (
            mean_prototypes.to(self.device) if mean_prototypes is not None else None
        )
        prototype_mask = (
            mean_prototype_mask.to(self.device, dtype=torch.bool)
            if mean_prototype_mask is not None
            else None
        )
...
def rename_dataset_metrics(metrics, prefix):
    ...
    return {
        ...
        f"{prefix}_mean_prototype_acc": metrics["global_test_mean_prototype_acc"],
    }
...
def fedavg_fixmatch(alpha, args=None):
    ...
    metrics = []
    mean_prototypes = None
    mean_prototype_mask = torch.zeros(args.num_classes, dtype=torch.bool)
    ...
    for round_id in progress:
        ...
        tasks = [
            ClientTask(
                ...
                global_prototypes=mean_prototypes,
                global_prototype_mask=mean_prototype_mask,
        ...
        mean_prototypes = server.aggregate_prototypes(
            ...
            previous=mean_prototypes,
        )
        mean_prototype_mask |= round_prototype_mask
        ...
        labeled_metrics = server.evaluate(
            ...
            mean_prototype_mask=mean_prototype_mask,
        )
        ...
        norm_metrics = {
            **prototype_norm_metrics(mean_prototypes, prefix="mean"),
        }
        row = {
            "round": round_id,
            **test_metrics,
        ...
        logger.info(
            ...
            display(test_metrics["global_test_mean_prototype_acc"]),
            display(client_metrics["labeled_global_model_acc"]),
            display(client_metrics["labeled_mean_prototype_acc"]),
            display(client_metrics["u_pool_global_model_acc"]),
            display(client_metrics["u_pool_mean_prototype_acc"]),
        )
        logger.info(
            "第 %d 轮各类均值原型 L2 范数：%s",
            round_id,
            np.round(
                [
                    norm_metrics.get(f"mean_prototype_l2_norm_class_{c}", np.nan)
