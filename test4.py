...
def dist_contrastive_loss(features, prototypes, labels, margin=0.0):
    ...
    if margin > 0:
        distances = distances + F.one_hot(
            labels, prototypes.size(0)
        ).to(features.device) * margin
...
class PLN(torch.nn.Module):
    def __init__(self, num_classes, width, feature_dim, depth=1, fixed=0, init_emb=0):
        ...
        initializers = {
            1: lambda x: torch.nn.init.uniform_(x, -0.1, 0.1),
            2: lambda x: torch.nn.init.normal_(x, 0.0, 0.1),
            3: lambda x: torch.nn.init.normal_(x, 0.0, 0.01),
            4: torch.nn.init.xavier_uniform_,
            5: torch.nn.init.xavier_normal_,
            6: lambda x: torch.nn.init.kaiming_uniform_(x, nonlinearity="linear"),
            7: torch.nn.init.orthogonal_,
        }
        if init_emb:
            initializers[init_emb](self.embeddings.weight)
        if fixed:
...
class Global:
    ...
    def aggregate_pln(self, local_states, sample_counts):
        result = copy.deepcopy(local_states[0])
        total = sum(sample_counts)
        for name in result:
            result[name] = sum(
                state[name] * count
                for state, count in zip(local_states, sample_counts)
            ) / total
        self.pln.load_state_dict(result)
        return result

    def download_pln_state(self):
        return {
    ...
    def evaluate(
        ...
        for images, labels in DataLoader(dataset, batch_size=batch_size):
            ...
            if pln_prototypes is not None:
                pln_is_correct = nearest(features, pln_prototypes) == labels
                pln_correct += pln_is_correct.sum().item()
                pln_rescued += (pln_is_correct & ~model_is_correct).sum().item()
        ...
        result = {
            ...
            "global_test_pln_prototype_acc": (
                pln_correct / total if pln_prototypes is not None else np.nan
            ),
            "global_test_pln_prototype_correct_given_model_wrong": (
                pln_rescued / model_errors
                if pln_prototypes is not None and model_errors
                else np.nan
            ),
        }
...
class Local:
    ...
    def train(
        ...
        self.pln.eval()
        all_classes = torch.arange(args.num_classes, device=self.device)
        with torch.no_grad():
            training_prototypes = self.pln(all_classes)
        ...
        for _ in range(args.local_epochs):
            ...
            for _ in range(local_steps):
                ...
                prototype_loss = dist_contrastive_loss(
                    features_x,
                    training_prototypes,
                    targets_x,
                    args.margin_pln,
        ...
        pln_dataset = Subset(
            labeled_dataset, range(len(labeled_dataset.indices))
        )
        pln_loader = DataLoader(
            pln_dataset,
            batch_size=args.batch_size_pln,
            shuffle=True,
        )
        for _ in range(args.epoch_pln):
            for images, labels in pln_loader:
                images, labels = images.to(self.device), labels.to(self.device)
                with torch.no_grad():
                    features, _ = self.local_model(images)
                learned_prototypes = self.pln(all_classes)
                pln_loss = dist_contrastive_loss(
                    features, learned_prototypes, labels, args.margin_pln
                )
...
class ClientTrainer:
    ...
    def train(self, task, labeled_view, unlabeled_view):
        ...
        params, pln_state, prototypes, counts, elapsed = self.local.train(
            task.args,
            labeled_view,
            unlabeled_view,
            eval_labeled,
...
def rename_dataset_metrics(metrics, prefix):
    ...
    renamed = {
        ...
        f"{prefix}_pln_prototype_correct_given_model_wrong": metrics[
            "global_test_pln_prototype_correct_given_model_wrong"
        ],
    }
    ...
    for name, value in metrics.items():
        if name.startswith(diagnostic_prefix):
            ...
            if short_name not in {
                ...
                "pln_prototype_acc",
                "pln_prototype_correct_given_model_wrong",
            }:
                renamed[f"{prefix}_{short_name}"] = value
    return renamed


def prototype_norm_metrics(prototypes, valid_mask, pln_prototypes):
    """返回每个类别原始聚合原型的 L2 范数。"""
    metrics = {}
    for class_id, valid in enumerate(valid_mask.tolist()):
        ...
        metrics[f"pln_prototype_l2_norm_class_{class_id}"] = pln_prototypes[
            class_id
        ].norm().item()
    return metrics
...
def fedavg_fixmatch(alpha, args=None):
    ...
    for round_id in progress:
        ...
        pln_state = server.download_pln_state()
        ...
        server.aggregate_pln(
            [result.pln_state for result in results], sample_counts
        )
        learned_prototypes = server.pln_prototypes()
        ...
        norm_metrics = prototype_norm_metrics(
            global_prototypes, global_prototype_mask, learned_prototypes
        )
        ...
        logger.info(
            ...
            display(test_metrics["global_test_pln_prototype_correct_given_model_wrong"]),
            display(client_metrics["labeled_raw_prototype_correct_given_model_wrong"]),
            display(client_metrics["labeled_pln_prototype_correct_given_model_wrong"]),
            display(client_metrics["u_pool_raw_prototype_correct_given_model_wrong"]),
            display(client_metrics["u_pool_pln_prototype_correct_given_model_wrong"]),
        )
        logger.info(
        ...
        logger.info(
            ...
            np.round(
                [
                    norm_metrics[f"pln_prototype_l2_norm_class_{class_id}"]
                    for class_id in range(args.num_classes)
                ],
                4,
            ).tolist(),
        ...
        if (
            ...
            torch.save(
                {"global": aggregated_params, "pln": server.download_pln_state()},
